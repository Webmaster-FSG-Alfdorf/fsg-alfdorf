...
function update() {
    currentMember.getRoles().then((roles) => {
        toggleMenu(roles, "276cacd9-b43e-4e4e-8e3f-92192eb8eba7", null, "Gästeverwaltung", "guests-management");
...
function toggleMenu(roles, roleID, dataSet, label, url) {
    ...
    if (showMenu && !menuShown) {
        ...
        if (dataSet) wixData.query(dataSet).descending("_updatedDate").limit(1).find().then((res) => {
